...
impl<I: Iterator<Item = Token>> Parser<I> {
    ...
    fn parse_transaction_argument(&mut self) -> Result<TransactionArgument> {
        Ok(match self.next()? {
            Token::U8(s) => {
                TransactionArgument::U8(s.parse().map_err(|e| anyhow::anyhow!("{:?}", e))?)
            }
            Token::U64(s) => {
                TransactionArgument::U64(s.parse().map_err(|e| anyhow::anyhow!("{:?}", e))?)
            }
            Token::U128(s) => {
                TransactionArgument::U128(s.parse().map_err(|e| anyhow::anyhow!("{:?}", e))?)
            }
            ...
            Token::Address(addr) => TransactionArgument::Address(
                AccountAddress::from_hex_literal(&addr).map_err(|e| anyhow::anyhow!("{:?}", e))?,
            ),
            Token::Bytes(s) => TransactionArgument::U8Vector(
