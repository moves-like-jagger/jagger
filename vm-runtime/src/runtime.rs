...
pub trait MoveRuntime {
    type Loader: Loader;

    fn new_session<'r, S: MoveResolver>(remote: &'r S) -> Session<'r, '_, S> {
        Self::new_session_with_extensions(remote, NativeContextExtensions::default())
    }

    fn new_session_with_extensions<'r, S: MoveResolver>(
        remote: &'r S,
        ...
        for module in &compiled_modules {
            ...
            if data_store.exists_module(&module_id)? && compat_check {
                ...
                let compat = Compatibility::check(
                    /* TODO verifier_config.treat_friend_as_private */
                    false, &old_m, &new_m,
                );
    ...
    fn deserialize_value(ty: &Type, arg: impl Borrow<[u8]>) -> PartialVMResult<Value> {
    ...
    fn execute_function_impl(
        ...
        let (mut dummy_locals, deserialized_args) =
            Self::deserialize_args(arg_types, serialized_args)
        ...
        let return_values = Interpreter::<Self::Loader>::entrypoint(
            func,
            ty_args,
            deserialized_args,
            data_store,
        ...
        let serialized_return_values = Self::serialize_return_values(&return_types, return_values)
            .map_err(|e| e.finish(Location::Undefined))?;
    ...
    fn execute_function(
        ...
        ) = Self::Loader::load_function(module, function_name, &ty_args, data_store)?;
    ...
    fn execute_script(
        ...
        ) = Self::Loader::load_script(script.borrow(), &ty_args, data_store)?;
        // execute the function
        Self::execute_function_impl(
            func,
            type_arguments,
