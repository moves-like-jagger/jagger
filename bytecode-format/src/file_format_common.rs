...
mod no_std {
    ...
    fn read_exact(src: &[u8], buf: &mut [u8]) -> anyhow::Result<()> {
        if buf.len() > src.len() {
            anyhow::bail!("failed to fill whole buffer");
        }
        let (a, _) = src.split_at(buf.len());
    ...
    fn read(src: &[u8], buf: &mut [u8]) -> anyhow::Result<usize> {
        let amt = core::cmp::min(buf.len(), src.len());
        let (a, _) = src.split_at(amt);
        ...
        T: AsRef<[u8]>,
    {
        ...
        /// just adapt to the std::io::Read
        pub fn read(&mut self, buf: &mut [u8]) -> anyhow::Result<usize> {
            // FIXME? usize occupies 4 bytes in wasm
            let n = read(self.remaining_slice(), buf)?;
            self.pos += n as u64;
            Ok(n)
        }
        ...
        pub fn read_exact(&mut self, buf: &mut [u8]) -> anyhow::Result<()> {
            let n = buf.len();
            read_exact(self.remaining_slice(), buf)?;
            self.pos += n as u64;
            Ok(())
        }
    }
